...
export default function ResultsPage() {
  ...
  // Cross-pathway notice: show if EB-1A and EB-2 NIW both score >= 65
  ...
  useEffect(() => {
    // Load results from sessionStorage (set by quiz completion)
    const stored = sessionStorage.getItem("passly_results");
    if (stored) {
      try {
  ...
  async function handleLeadSubmit(e: React.FormEvent) {
    ...
    setLeadLoading(true);
    try {
      const sessionId = sessionStorage.getItem("passly_session");
      await fetch("/api/leads", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, locale, sessionId }),
      });
    ...
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <CTAButton href={`/${locale}/quiz`}>{t("retake")}</CTAButton>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <div className="max-w-2xl mx-auto px-4 pb-12 text-center">
        <Link href={`/${locale}/quiz`} className="text-sm text-muted hover:text-primary transition-colors">
          {t("retake")} →
        </Link>
      </div>
    </div>
