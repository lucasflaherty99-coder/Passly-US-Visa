...
const EXP_OPTIONS = [
  { value: 0, label: "lessThan1", years: 0 },
  { value: 2, label: "years13", years: 2 },
  { value: 4, label: "years36", years: 4 },
  { value: 8, label: "years610", years: 8 },
  { value: 12, label: "years10plus", years: 12 },
];
...
export function ProfessionalStep() {
  ...
  function handleNext() {
    const nextStep = getNextStep(STEP.PROFESSIONAL, state.answers);
    if (nextStep !== null) {
  ...
  function handleBack() {
    const prevStep = getPrevStep(STEP.PROFESSIONAL, state.answers);
    if (prevStep !== null) {
          ...
          <div className="space-y-2">
            {EXP_OPTIONS.map((opt) => (
              <OptionButton
                ...
                label={t(opt.label)}
                selected={state.answers.yearsExperience === opt.years}
                onClick={() => setAnswer("yearsExperience", opt.years)}
              />
            ))}
          </div>
        </div>
