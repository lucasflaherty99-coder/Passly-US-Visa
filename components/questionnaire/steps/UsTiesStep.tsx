...
export function UsTiesStep() {
  ...
  function handleNext() {
    const nextStep = getCheckNextStep(CHECK_STEP.US_TIES, state.answers);
    if (nextStep !== null) {
  ...
  function handleBack() {
    const prevStep = getCheckPrevStep(CHECK_STEP.US_TIES, state.answers);
    if (prevStep !== null) {
  ...
  const canAdvance = state.answers.hasOverstay !== undefined &&
    state.answers.hasDenial !== undefined &&
    state.answers.permanentJobSponsor !== undefined &&
    state.answers.temporaryJobSponsor !== undefined;

  return (
    <QuestionCard
      step={CHECK_STEP.US_TIES}
      question={t("question")}
      subtitle={t("subtitle")}
      onBack={handleBack}
    >
      <div className="space-y-6">
        ...
        <YesNoQ
          ...
          yes={t("yes")} no={t("no")}
        />
        {/* Permanent sponsor (green card) */}
        <YesNoQ
          question={t("permanentSponsor")}
          value={state.answers.permanentJobSponsor}
          onChange={(v) => {
            const isYes = v === "yes";
            setAnswer("permanentJobSponsor", isYes);
            // Keep legacy hasUsSponsor in sync
            if (isYes) setAnswer("hasUsSponsor", true);
            else if (!state.answers.temporaryJobSponsor) setAnswer("hasUsSponsor", false);
          }}
          yes={t("yes")} no={t("no")}
        />
        {/* Temporary sponsor (H-1B / L-1) */}
        <YesNoQ
          question={t("temporarySponsor")}
          value={state.answers.temporaryJobSponsor}
          onChange={(v) => {
            const isYes = v === "yes";
            setAnswer("temporaryJobSponsor", isYes);
            if (isYes) setAnswer("hasUsSponsor", true);
            else if (!state.answers.permanentJobSponsor) setAnswer("hasUsSponsor", false);
          }}
          yes={t("yes")} no={t("no")}
        />
        <YesNoQ
